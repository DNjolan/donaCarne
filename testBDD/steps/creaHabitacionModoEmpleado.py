...
@given(u'test 8 Que pueda navegar por la página "{url}"')
def test_8_abrir_navegador_dos(context,url):
    set = webapp.setUp()
    driver.get(url)

@when(u'test 8 hago click en el botón iniciar sesión "{enlace}"')
def test_8_abrir_pagina_dos(context, enlace):
    driver.implicitly_wait(30)
    driver.find_element(By.XPATH,enlace).click()

@when('test 8 ingresar datos de administrador')
def test_8_iniciar_sesion_dos(context):
...
@when(u'test 8 hacer click en botón Administrador habitaciones "{enlace}"')
def test_8_boton_administrador_habitacion_dos(context, enlace):
    driver.implicitly_wait(30)
    driver.find_element(By.XPATH,enlace).click()

@when(u'test 8 hacer click en botón Crear habitación "{enlace}"')
def test_8_boton_crear_habitacion_dos(context, enlace):
    driver.implicitly_wait(30)
    driver.find_element(By.XPATH,enlace).click()

@when(u'test 8 ingresar datos de la habitación')
def test_8_ingresar_datos_dos(context):
...
def test_8_creacion_habitacion_dos(context):
    driver.implicitly_wait(30)
